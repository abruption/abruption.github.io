...
author: abruption
date: 2026-03-11 02:00:00 +0900
categories: [Productivity, Obsidian]
tags: [obsidian, claude-code, automation, sqlite, skills, rules, pkm]
---

> **2026-09-11 업데이트**: 이 글을 쓸 때는 볼트와 SQLite를 같은 환경에서 직접 이중 기록했다. 지금은 문서와 첨부파일은 iCloud Drive에 두고, DB는 중앙 한 곳만 사용한다. Mac이나 US 서버에서 `vault` 명령을 실행하면 중앙 런타임으로 전달되고, DB 조회·기록·인덱싱·렌더링이 그쪽에서 처리된다. 기기별로 SQLite 파일을 열거나 복사하는 방식은 더 이상 쓰지 않는다.
...
Claude Code를 볼트에 연결하면서 상황이 바뀌었다. 마크다운 문서만으로 관리하기 어려웠던 정량 데이터를 DB에도 기록하고, 반복 작업은 Skills로 자동화하고, 문서 작성 규칙은 Rules로 강제하는 구조를 만들었다. 이 글을 쓸 당시에는 25개의 Skill, 15개의 Rule, 14개의 SQLite CLI 스크립트가 볼트 안에서 돌아가고 있었다.

구현 코드를 전부 설명하기보다는, 어떤 구조를 만들었고 어디서 운영 방식이 바뀌었는지를 적어둔다.
...
**자산 스냅샷** — 전체 자산 현황을 스냅샷으로 기록한다. 계좌별 잔액을 입력하면 문서 생성과 DB 기록, 인덱스 전파까지 한 번에 처리한다.

**투자 현황 업데이트** — 증권사 API로 시세를 조회해서 보유 종목의 평가금액·수익률을 자동 갱신한다. 3개 계좌 문서 + 통합 요약까지 한 번에 업데이트.

**지출/수입 기록** — 거래 내역을 입력하면 월별 기록 문서에 추가하고, DB에도 기록하고, 인덱스 통계를 갱신한다.

### 생활 계열

**여행 문서 생성** — 날짜와 도시를 입력하면 항공편·숙소·일정·경비 섹션이 포함된 여행 문서 템플릿을 생성한다.

**영어 연습** — 일상 주제로 영어 회화 질문을 생성하고, 답변에 대한 피드백을 제공한다. 연습 기록이 DB에 쌓여서 복습 모드에서 과거에 틀린 표현을 다시 출제한다.
...
## 문서는 iCloud, 정량 데이터는 중앙 DB

이 구조에서 가장 고민을 많이 한 부분은 마크다운 문서와 정량 데이터를 어떻게 같이 가져갈지였다. Obsidian은 문서를 읽고 쓰기에는 좋지만, "최근 3개월 지출 추이"나 "카드별 마일리지 누적"을 계산하기에는 불편하다. 반대로 DB만 쓰면 Obsidian에서 읽기 좋은 기록이 남지 않는다.

처음에는 **Dual-Write**로 풀었다. Skill이 로컬 SQLite에 기록하고, 그 결과를 마크다운의 생성 블록으로 다시 그리는 방식이었다. 당시에는 간단했지만, 여러 기기에서 iCloud를 통해 볼트를 쓰기 시작하니 DB까지 같이 동기화하는 건 오래 가져갈 구조가 아니었다.

지금은 역할을 나눴다.

```
사용자 또는 Skill
  ├─ 문서·첨부파일 → iCloud Drive
  └─ `vault run` → 중앙 런타임 → 중앙 SQLite
                         ├─ `vault query`  (검색·조회)
                         ├─ `vault index`  (문서 인덱스)
                         └─ `vault render` (생성 블록 반영)
```

Mac이나 US 서버에서 `vault`를 실행해도 로컬 DB를 열지 않는다. 명령이 중앙 런타임으로 전달되고, 그곳의 DB 한 곳만 읽고 쓴다. 기록 결과에서는 DB 저장 여부와 문서 반영 여부도 따로 본다. DB는 커밋됐지만 문서가 아직 `pending`이면 같은 기록을 다시 넣지 않고 렌더만 재시도한다.

이렇게 바꾸고 나서 기기마다 서로 다른 SQLite 파일이 생기는 문제는 사라졌다. 대신 중앙 런타임에 접근하지 못하면 조회나 기록을 바로 끝낼 수 없다는 트레이드오프가 생겼다. 그래도 데이터가 조용히 갈라지는 것보다는 지금 구조가 낫다고 판단했다.

## 이 구조가 주는 것

몇 달 운영하면서 실제로 체감한 건 이렇다.

**일관성**. Rules가 강제하는 프론트매터, 파일명, 인덱스 동기화 덕분에 볼트가 무질서해지지 않는다. 200개 넘는 문서가 있지만 "어디에 뭐가 있는지" 항상 파악 가능하다.

**데이터 활용**. 마크다운만으로는 불편했던 "월별 지출 추세", "자산 변동 추이", "카드별 마일리지 누적" 같은 분석을 DB 쿼리로 할 수 있다. 기록하는 노력은 비슷한데, 나중에 꺼내 쓰는 방법이 많아진다.
...
**초기 설정 비용**. Rules 15개, Skills 25개를 한 번에 만든 건 아니다. 필요할 때마다 하나씩 추가했는데도 누적 시간은 꽤 들었다. "그냥 노트 쓰는 것"과는 결이 다른 작업이다.
...
**중앙 런타임 의존성**. 지금은 DB를 iCloud에 같이 두지 않는다. iCloud는 문서와 첨부파일만 맡고, DB 조회·기록·인덱싱·렌더링은 `vault`를 통해 중앙 런타임에서 처리한다. 그래서 기기별 DB 충돌은 피했지만, 중앙 런타임이 잠시 닿지 않으면 작업도 잠시 멈춘다.
...
Obsidian 볼트에 Claude Code를 연결하면, 노트 앱이 개인용 데이터 플랫폼에 가까워진다. 지금 구조를 한 줄로 줄이면 이렇다.

- **Rules** — 문서 구조, 동기화, 접근 권한을 자동으로 강제
- **Skills** — 반복 작업을 자연어 한마디로 실행
- **중앙 DB** — 정량 데이터를 한 곳에 기록하고 쿼리
- **Telegram** — 장소에 구애받지 않는 볼트 접근 채널

처음부터 거창하게 만들 필요는 없다. 나도 처음에는 프론트매터 규칙 하나와 자산 스냅샷 Skill 하나로 시작했다. 반복되는 작업이 보일 때마다 Skill을 추가했고, 문서가 어지러워질 때마다 Rule을 하나씩 잡았다. 나중에 데이터가 여러 기기로 퍼지기 시작했을 때 DB 운영 방식도 다시 바꿨다. 이 구조는 완성품이라기보다, 실제로 쓰면서 계속 고치는 작업에 가깝다.
