...
아래는 그 과정에서 실제로 멈췄던 지점과, 그때 바꾼 설계를 적은 글이다.
...
그래서 `BaseAgent`라는 추상 클래스를 만들었다. GoF의 Template Method 패턴이다. 공통 흐름은 한 곳에 두고, 에이전트마다 달라지는 부분만 오버라이드하게 했다.
...
이 구조의 효과는 나중에 Frontend 에이전트를 추가할 때 바로 보였다. 메서드 몇 개만 구현하니 스트리밍, 재시도, 상태 복구가 같이 따라왔다. 새 에이전트를 붙이는 데 하루가 걸리지 않았다.
...
지금 다시 만든다면 에이전트 수를 늘리는 일보다 실패 지점을 먼저 설계할 것 같다. 이 시스템에서 시간을 가장 많이 잡아먹은 건 프롬프트를 잘 쓰는 일이 아니라, 중간에 실패했을 때 어디까지 되돌릴지 정하는 일이었다.
