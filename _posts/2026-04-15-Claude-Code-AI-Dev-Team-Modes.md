...
아래는 세 모드가 실제로 어떻게 갈렸는지, 그리고 돌려보면서 어디서 문제가 났는지를 적은 글이다.
...
단순해 보이지만 여기서 잘못 고르면 시간을 두 배로 쓴다. 실제로 모드를 잘못 골라서 만든 산출물을 버리고 다시 시작한 적도 있다.
...
모드가 달라도 마지막까지 가져가는 Phase 구조는 같았다.
...
타입 체크만 통과했다고 끝내면 안 됐다. 쿼리 바인딩이 안 맞아서 런타임에 터지는 경우도 있었고, 테스트는 통과하지만 중복 코드가 잔뜩 남는 경우도 있었다. 그래서 세 단계를 모두 통과한 뒤에야 작업을 끝낸 것으로 봤다.
...
처음부터 팀 모드로 갈 필요는 없었다. 단일 Agent로 참조 코드를 하나 만들고, 규모가 커졌을 때 팀을 붙이는 순서가 훨씬 덜 흔들렸다. 반복되는 실수는 그때그때 프롬프트에 남겼다.
