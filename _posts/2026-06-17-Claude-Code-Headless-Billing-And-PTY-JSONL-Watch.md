...
처음 설계는 늘 "지금 가장 쉬운 방법"으로 기운다. 우리에게는 `claude -p`가 그랬다. 정책이 그대로 시행됐다면 시연 자리에서 사용자의 monthly credit가 줄어드는 걸 보고서야 부랴부랴 갈아엎었을 가능성이 크다.

이번 일에서 남은 건 외부 정책에 설계를 너무 깊게 걸어두지 말자는 것이다. 정책 변화 자체는 막을 수 없지만, 그 한 줄 때문에 전체 채널을 갈아엎어야 하는 구조는 피할 수 있다. 우리는 5월에 그 한 줄을 보고 방향을 바꿨고, 6월에 정책이 보류된 뒤에도 그 판단을 되돌리지는 않았다.
