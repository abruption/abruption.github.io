...
두 CLI는 다루는 도메인이 완전히 다르지만(하나는 AI 에이전트 쿼타, 하나는 클라우드 비용) 막상 만들어보니 비슷한 곳에서 걸렸다. 자격증명은 OS 키링을 먼저 보고, 배포는 초록불만 믿지 않고, 이상한 버그가 나오면 코드보다 런타임과 OS를 먼저 의심하게 됐다. 특히 `AggregateError`의 빈 메시지와 심링크 경로 문제는 로컬에서 아무리 봐도 안 보이던 종류였다.

두 도구 모두 의존성을 많이 늘리지 않았다. `oci-cost-cli`는 자격증명 저장용 라이브러리 하나가 전부다. 결국 둘 다 내가 매번 손으로 하던 일을 대신하는 정도의 작은 도구로 남았고, 그 정도가 오히려 마음에 든다. 화려한 기능보다 실행했을 때 예상한 결과가 나오는지가 더 중요했다.
