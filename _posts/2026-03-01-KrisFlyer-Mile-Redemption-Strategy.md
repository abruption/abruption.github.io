...
작년 5월에 신한카드 싱가포르항공 더 베스트를 발급받았다. 해외 결제가 잦아서 국내 항공사 마일보다 SIA 쪽이 낫겠다고 봤다. 결제금액 1,500원당 2마일(기본)이고, 부스터마일즈까지 합치면 일반 국내 결제도 3마일/1,500원이 된다는 점이 마음에 들었다.
...
그래서 공제표를 직접 뜯어보고, 목적지별로 몇 가지 조합을 시뮬레이션해봤다. 그 결과를 적어둔다. 나중에 다시 발권할 때 내가 보려고 만든 표이기도 하다.
...
|------|---:|---:|---:|---:|
| 시드니 비즈니스 | 126,000 | 38,000 | **88,000** | 가장 유리 |
...
처음에는 공제표가 숫자만 잔뜩 적힌 표처럼 보였다. 그런데 Zone 구조를 알고 나니 출발지를 바꾸는 것만으로 차이가 꽤 커졌다. SIA와 Scoot를 같은 프로그램 안에서 조합할 수 있다는 점도 생각보다 쓸모가 있었다.

당장은 시드니 Scoot 이코노미 발권부터 해볼 생각이다. 장기적으로는 SQ 비즈니스로 시드니까지 가보고 싶다. 비즈니스 좌석에서 먹는 Book the Cook이 그렇게 대단하다던데, 실제로 타보게 되면 그때 후기를 써보겠다.
